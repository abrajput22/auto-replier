        ...
        if data.get("object") == "instagram":
            ...
            for entry in data.get("entry", []):
                ...
                if messaging_events:
                    ...
                    tasks = [process_message(m) for m in messaging_events]
                ...
                if changes:
                    ...
                    for change in changes:
                        ...
                        print(f"Change field: {field}, value: {value}")

                        if field == "comments":
                            print("Top-level comment event detected")
                            try:
                                await process_comment(value)
                            except Exception as e:
                                print(f"Error processing top-level comment: {e}")

                        elif field == "media" and "comment_id" in value:
                            print("Reply-to-comment event detected")
                            try:
                                await process_comment(value)
                            except Exception as e:
                                print(f"Error processing reply comment: {e}")

                        else:
                            print(f"Ignoring field: {field}")
                else:
            ...
            for entry in data.get("entry", []):
                for messaging in entry.get("messaging", []):
                    try:
                        ...
                        print(f"Error processing page message: {e}")
